...
class TrainingPipeline:
    def __init__(self, dataset, backbone_model, batch_size=32, k_folds=5, num_epochs=10, init_lr=0.001, class_weights = None, train_backbone = True):
        ...
        self.perf_evaluator_model_global = PerformancePredictor(backbone_model, 0.5)
    ...
    def save_models(self, output_dir):
        ...
        for key, model in self.best_models.items():
            ...
            if key == 'best_model_criteria1':
                self.perf_evaluator_model_global.criteria1_conv = model.criteria1_conv
                self.perf_evaluator_model_global.criteria1_lin = model.criteria1_lin
            if key == 'best_model_criteria2':
                self.perf_evaluator_model_global.criteria2_conv = model.criteria2_conv
                self.perf_evaluator_model_global.criteria2_lin = model.criteria2_lin
            if key == 'best_model_criteria3':
                self.perf_evaluator_model_global.criteria3_conv = model.criteria3_conv
                self.perf_evaluator_model_global.criteria3_lin = model.criteria3_lin
            if key == 'best_model_criteria4':
                self.perf_evaluator_model_global.criteria4_conv = model.criteria4_conv
                self.perf_evaluator_model_global.criteria4_lin = model.criteria4_lin
        torch.save(self.perf_evaluator_model_global, model_path)
    ...
    def plot_metrics(self, save_folder, criteria_names = ['Relative position and orientation between neighboring buildings', 'Position and orientation of buildings in relation to closest roads', 'Integrity of edges', 'Straightness of edges']):
        ...
        for j, name in enumerate(criteria_names):
            plt.plot(self.H_val[self.best_performing_folds[j]][f"total_accuracy_criteria{j+1}"], label='Accuracy')
            plt.xlabel('Epochs')
            plt.ylabel('Accuracy')
            plt.title(f'Accuracy Plot for the criterion: {name}')
            plt.legend()
            plt.savefig(os.path.join(save_folder, f'best_performing_folds_accuracies_{name}.png'))
            plt.close()

            plt.plot(self.H_val[self.best_performing_folds[j]][f"total_loss_criteria{j+1}"], label='Loss')
            plt.xlabel('Epochs')
            plt.ylabel('Loss')
            plt.title(f'Loss Plot for the criterion: {name}')
            plt.legend()
            plt.savefig(os.path.join(save_folder, f'best_performing_folds_losses_{name}.png'))
            plt.close()
