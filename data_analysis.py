...
for i in range(len(dataset)):
    print(i)
    aux_list = []
    aux_list.append(float(dataset[i][1]["Relative position and orientation between neighboring buildings"]))
    aux_list.append(float(dataset[i][1]["Position and orientation of buildings in relation to closest road/s"]))
    aux_list.append(float(dataset[i][1]["Building types in relation to underlying terrain type"]))
    aux_list.append(float(dataset[i][1]["Integrity of edges"]))
    aux_list.append(float(dataset[i][1]["Straightness of edges"]))
    aux_list.append(float(dataset[i][1]["Size relative to type"]))
    aux_list.append(float(dataset[i][1]["Conservation of color codi"]))
    avg = 0
    for i in aux_list:
        avg += i/len(aux_list)
    aux_list.append(avg)
